...
- Note any existing design assets: `.hallmark/tokens.css`, a tailwind config with brand values, a logo, brand colours mentioned in `README`, a marketing screenshot.
...
### 2. Produce `.hallmark/design.md`

Write a single file at `.hallmark/design.md` (or `.hallmark/DESIGN.md` — match the project's existing case convention). Never write it at the project root. This file is the **one source of truth** every subsequent page redesign reads. Format:
...
## Spacing
4-point named scale. The values are in `.hallmark/tokens.css`. Pages must use named
...
@theme {
  ...
  --text-md:       1.125rem;
  --ease-out:      cubic-bezier(0.16, 1, 0.3, 1);
  /* mirror the rest of .hallmark/tokens.css with `--spacing-*` for Tailwind's spacing utilities */
}
```
...
Wait for confirmation. Only after the user confirms (or says "go ahead") do you write `.hallmark/design.md` (and `.hallmark/tokens.css`) and start redesigning pages.

### 3. Redesign each page reading from `.hallmark/design.md`

For each target page:

- **Read `.hallmark/design.md` first.** It is now the rule of the project; the per-build references in [`references/`](../) defer to it. Where `design.md` and the references conflict, `design.md` wins.
...
- Stamp every page's CSS with: `/* Hallmark · genre: <genre> · macrostructure: <name> · design-system: .hallmark/design.md · designed-as-app */`. The `designed-as-app` flag tells future Hallmark runs to read `.hallmark/design.md`, not invent a new system.
...
Pages that drift from `.hallmark/design.md` are slop. The audit verb flags `design.md` drift as a critical structural finding (`stamp-vs-design.md disagreement`).

### 5. When to amend `.hallmark/design.md` instead of overriding

If a page genuinely needs something `design.md` doesn't allow (e.g. a marketing landing for a new sub-product wants a different theme), the rule is **amend `.hallmark/design.md` first**, not override locally. Add an explicit per-page allowance or a `## Variants` section. The file evolves; per-page overrides do not.
...
**Project-level check.** Before treating this as a true single-page redesign, look for `.hallmark/design.md` (legacy root `design.md` still counts — migrate into `.hallmark/` on next write). If it exists, the project is being designed as an app and **the single-page rules don't apply** — read it and follow it instead. The diversification rule reverses (consistency wins). If you actually want to break from the locked system, *update `.hallmark/design.md` first*, then redesign.
