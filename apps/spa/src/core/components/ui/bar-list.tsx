    ...
    <div ref={ref} className={twMerge('flex justify-between space-x-6', className)} {...props}>
        ...
        {sortedData.map((item, index) => (
          <Component
            ...
            className={twJoin(
              ...
              `
                focus:ring-2 focus:inset-ring focus:ring-ring/20
                focus:inset-ring-ring focus:outline-hidden
              `,
              onValueChange
                ? `
                  -m-0! cursor-pointer
          ...
          >
            <div
              className={twJoin(
                `
                  flex items-center rounded-sm bg-primary/15 transition-all
                  dark:bg-primary/20
                `,
                rowHeight,
                onValueChange
                  ? `
                    ...
                    dark:group-hover:bg-primary/35
                  `
                  : '',
            ...
            >
              <div className="absolute left-2 flex max-w-full pr-2">
                {item.href
                  ...
                  : (
                      <p className="truncate text-sm whitespace-nowrap text-fg">{item.name}</p>
                    )}
              </div>
            </div>
          </Component>
      ...
      <div>
        {sortedData.map((item, index) => (
          ...
          >
            <p className={`
              truncate text-sm leading-none whitespace-nowrap text-fg
            `}
            >
