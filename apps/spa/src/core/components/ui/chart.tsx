...
export const valueToPercent = (value: number) => `${(value * 100).toFixed(0)}%`;
...
const XAxis = ({
  ...
  return (
    <XAxisPrimitive
      className={twMerge(
        "text-xs text-muted-fg **:[text]:fill-muted-fg",
        className
      )}
...
const DEFAULT_Y_AXIS_DOMAIN: YAxisPrimitiveProps["domain"] = ["auto", "auto"];

const getLegendAlignClass = (
...
interface ChartTooltipPayloadItemProps<
  TValue extends ValueType,
  TName extends NameType,
> {
  color?: string;
  formatter?: TooltipContentProps<TValue, TName>["formatter"];
  hideIndicator: boolean;
  index: number;
  indicator: "dashed" | "dot" | "line";
  item: NonNullable<TooltipContentProps<TValue, TName>["payload"]>[number];
  nameKey?: string;
  nestLabel: boolean;
  tooltipLabel: React.ReactNode;
}
const getTooltipIndicatorClassName = (
  indicator: "dashed" | "dot" | "line",
  nestLabel: boolean
) =>
  twMerge(
...
const ChartTooltipIndicator = ({
  ...
  return (
    <div
      className={getTooltipIndicatorClassName(indicator, nestLabel)}
      style={
        {
...
const ChartTooltipItemDetails = ({
  ...
  >
    <div className="grid gap-1.5">
      ...
      <span className="text-muted-fg">{itemConfig?.label || itemName}</span>
    </div>
    {itemValue ? (
...
>({
  ...
  nestLabel,
  tooltipLabel,
}: ChartTooltipPayloadItemProps<TValue, TName>) => {
