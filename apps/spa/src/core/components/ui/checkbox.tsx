...
const getCheckIndicator = (isSelected: boolean, isIndeterminate: boolean) => {
  if (isIndeterminate) {
    return <MinusIcon data-slot="check-indicator" />;
  }
  if (isSelected) {
    return <CheckIcon data-slot="check-indicator" />;
  }
  return null;
};

export const Checkbox = ({ className, ...props }: CheckboxButtonProps) => (
  <CheckboxButton
    className={cx(
  ...
  >
    {composeRenderProps(
      ...
      (children, { isSelected, isIndeterminate, isInvalid }) => {
        const indicator = getCheckIndicator(isSelected, isIndeterminate);
        return (
          <div
            className={twMerge(
