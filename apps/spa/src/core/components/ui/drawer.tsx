...
const getDrawerSideClasses = (side: DrawerSide, isFloat: boolean) => {
  if (side === "top") {
    return isFloat
...
const getDrawerAxisOffset = (side: DrawerSide) => {
...
const getDrawerDragElastic = (side: DrawerSide) => ({
...
const shouldCloseDrawerOnDrag = (
  side: DrawerSide,
  offset: { x: number; y: number },
  velocity: { x: number; y: number }
) => {
  if (side === "bottom") {
    return velocity.y > 150 || offset.y > screen.height * 0.25;
  }
  if (side === "top") {
    return velocity.y < -150 || offset.y < screen.height * 0.25;
  }
  if (side === "left") {
    return velocity.x < -150;
  }
  if (side === "right") {
    return velocity.x > 150;
  }
...
const DrawerContent = ({
  ...
  const onOpenChange = props.onOpenChange ?? overlayState.setOpen;
  const axisOffset = getDrawerAxisOffset(side);
  return (
    <AnimatePresence>
      {isOpen && (
        <DrawerOverlay
          ...
          {({ state: modalState }) => (
            <DrawerRoot
              className={twJoin(
                ...
                getDrawerSideClasses(side, isFloat),
                className
              )}
              animate={{ x: 0, y: 0 }}
              initial={axisOffset}
              ...
              onDragEnd={(_, { offset, velocity }) => {
                if (shouldCloseDrawerOnDrag(side, offset, velocity)) {
                  modalState.close();
                }
              }}
              dragElastic={getDrawerDragElastic(side)}
              dragPropagation
            >
              <Dialog
                aria-label="Drawer"
