...
const fieldGroupStyles = tv({
  base: [
    `
      group flex h-10 items-center overflow-hidden rounded-lg border
      border-input shadow-xs transition duration-200 ease-out
    `,
    `
      relative
      focus-within:ring-4
      group-invalid:focus-within:border-danger
      group-invalid:focus-within:ring-danger/20
    `,
    `
      [&>[role=progressbar]:first-child]:ml-2.5
      [&>[role=progressbar]:last-child]:mr-2.5
    `,
    `
    ...
    `,
    `
      [&>button:has([data-slot=icon])]:absolute
      [&>button:has([data-slot=icon]):first-child]:left-0
      [&>button:has([data-slot=icon]):last-child]:right-0
    `,
    `
      *:data-[slot=icon]:pointer-events-none *:data-[slot=icon]:absolute
      *:data-[slot=icon]:top-[calc(var(--spacing)*2.7)] *:data-[slot=icon]:z-10
      *:data-[slot=icon]:size-4 *:data-[slot=icon]:text-muted-fg
    `,
    `
    ...
    `
      [&:has([data-slot=icon]+[role=group])]:pl-6
      [&:has([role=group]+[data-slot=icon])]:pr-6
    `,
    'has-[[data-slot=icon]:last-child]:[&_input]:pr-7',
    `
      *:[button]:h-8 *:[button]:rounded-[calc(var(--radius-sm)-1px)]
      *:[button]:px-2.5
    `,
    `
      [&>button:first-child]:ml-[calc(var(--spacing)*0.7)]
