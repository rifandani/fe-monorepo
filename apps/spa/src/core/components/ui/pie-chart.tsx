...
const EMPTY_PIE_DATA: BaseChartProps["data"] = [];
const defaultPieValueFormatter = (value: number) => value.toString();

const getPieConfigKey = (datum: PieChartDatum): string | undefined => {
          ...
          {children || (
            ...
            >
              {data.map((datum: PieChartDatum, index: number) => {
                const configKey = getPieConfigKey(datum);
                return (
                  <Cell
                    key={`cell-${index}`}
                    fill={getColorValue(
