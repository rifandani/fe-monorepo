...
const DEFAULT_Y_AXIS_DOMAIN: [string, string] = ["auto", "auto"];
...
const getLegendAlignClass = (align: LegendProps["align"]) => {
  if (align === "right") {
    return "justify-end";
  }
  if (align === "left") {
...
const renderTooltipPayloadItem = <
  TValue extends ValueType,
  TName extends NameType,
>({
  color,
    ...
    >
      {payload.map((item: LegendPayload) => {
        ...
        return (
          ...
          >
            {renderLegendItemIcon(itemConfig, hideIcon, item.color)}
            {itemConfig?.label}
          </ToggleButton>
        );
      })}
