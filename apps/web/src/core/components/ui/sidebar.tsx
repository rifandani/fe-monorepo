...
import { Button as Trigger } from "react-aria-components/Button";
...
const persistSidebarOpenState = async (openState: boolean) => {
  try {
    ...
    // Cookie Store API unavailable or rejected.
  }
};
...
const SidebarProvider = ({
  ...
  const setOpen = useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      void persistSidebarOpenState(openState);
    },
    [setOpenProp, open]
  );
  ...
  const link = (
    <Link
      ...
      className={composeRenderProps(
        ...
        (
          resolvedClassName,
          { isFocusVisible, isPressed, isHovered, isDisabled }
        ) =>
          twMerge(
  ...
  return (
    <Tooltip delay={0}>
      ...
      <TooltipContent
        ...
        placement="right"
        arrow
        hidden={!isCollapsed || isMobile || !resolvedTooltip}
        {...resolvedTooltip}
      />
    </Tooltip>
  );
    ...
    <Heading level={3}>
      <Trigger
        ...
        className={composeRenderProps(
          ...
          (
            resolvedClassName,
            { isPressed, isFocusVisible, isHovered, isDisabled }
          ) =>
            twMerge(
...
const SidebarTree = <T extends object>({
  className,
  selectionMode = "none",
  ...props
}: SidebarTreeProps<T>) => (
