 ...
 *
 * @property {number} totalHits - The number of hits for that client so far.
 * @property {Date | undefined} resetTime - The time when the counter resets.
 */
...
export interface ConfigType<P extends string = string> {
   ...
   * Defaults to `5`.
   */
  limit: number | ((context: Map<string, unknown>) => Promisify<number>);
  /**
