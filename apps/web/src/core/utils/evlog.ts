...
import { createRequestSizeEnricher, createTraceContextEnricher, createUserAgentEnricher } from 'evlog/enrichers'
...
const enrichers = [createUserAgentEnricher(), createRequestSizeEnricher(), createTraceContextEnricher()]
...
const drain = pipeline(createOTLPDrain({
...
export const { register: evlogRegister, onRequestError: evlogOnRequestError } = createInstrumentation({
  service: SERVICE_NAME,
  drain,
  captureOutput: true,
})
