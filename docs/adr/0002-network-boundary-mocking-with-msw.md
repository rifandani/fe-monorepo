...
**2026-09-11 — the complete set is five files.** `packages/core/src/services/http.unit.test.ts` now fakes at the Network Boundary, so the list under [Scope](#scope) is `packages/core/src/apis/{auth,better-auth,cdn}.unit.test.ts`, `apps/expo/src/user/apis/user.unit.test.ts`, and `packages/core/src/services/http.unit.test.ts`.
